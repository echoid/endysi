...
def _plotAllTrajectories(filename, data, mols, colours=None, ddpi=120):
    ...
    fig = plt.figure()
    for mol in mols:
        plt.plot(data['time'], data[mol], colours[count], label=mol.split('_')[0])
        count += 1
...
class Experiment:
    ...
    def loadData(self):
        ...
        self.equilData = np.genfromtxt('%s_equil.gdat' % self.model.filePath, names=True)
        self.perturbData = np.genfromtxt('%s_perturb.gdat' % self.model.filePath, names=True)
        
        # Get and save molecule names
        self.mols = [name for name in self.equilData.dtype.names if name != 'time']
    ...
    def makeFoldPlots(self, ddpi=120):
        ...
        for pair in self.corrMols['ceRNA']:
            plt.plot(self.equilData[pair[0]], self.equilData[pair[1]], colours[i])
            i += 1
        ...
        fig = plt.figure()
        for pair in self.corrMols['ceRNA']:
            plt.plot(self.perturbData[pair[0]], self.perturbData[pair[1]], colours[i])
            i += 1
...
class Endysi:
    def __init__(self, m, n, k, size, method, tEnd, outFreq, paramDict, timestamp=None):
        ...
        self.createLoggers()
        self.logger.info('Initializing system to run {0} networks with M={1}, N={2}, K={3}'.format(size, m, n, k))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', type=int, help='The number of models in the ensemble')
    parser.add_argument('--method', type=str, help='Simulation method: ode or ssa')
