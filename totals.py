...
def doit(m, n, k, s, timestamp, rootDir=None):
    ...
    for i in range(1, s + 1):
        ...
        miDeg = 0
        for pair in params:
            if pair[0].startswith('pT'):
                ceTrans += float(pair[1])

            elif pair[0].startswith('dT'):
                ceDeg += float(pair[1])

            elif pair[0].startswith('pR'):
...
if __name__ == '__main__':
    ...
    if args.root == 'None':
        ...
        rd = '/run/media/matt/Research/results/ceRNA/endysi'
        doit(args.m, args.n, args.k, args.s, args.time, rootDir=rd)